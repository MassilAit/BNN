...
def plot_gradients(path, function : str = "ReLu"):    
    with open(path, 'r') as file:
        data = json.load(file)
    ...
    # Extract activations for each input
    for input_key, epochs_data in data.items():
        layer1_activations[input_key] = [entry["model.linear_0.weight"][0] for entry in epochs_data]  # Layer 1 (ReLU)
        layer3_activations[input_key] = [entry["model.linear_1.weight"][0] for entry in epochs_data]  # Layer 3 (Sigmoid)
