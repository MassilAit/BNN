...
def _is(node, kind):
    return isinstance(node, tuple) and node and node[0] == kind
...
# ───────────────────── New k-input cost (tree) ──────────────────────────
def k_input_cost_tree(n):
    """AND/OR with k inputs costs k; NOT costs 0."""
    if _is(n, 'var'): return 0
    if _is(n, 'not'): return k_input_cost_tree(n[1])  # NOT costs 0
    if _is(n, 'and'):
        kids = _flatten_and(n)
        return len(kids) + sum(k_input_cost_tree(k) for k in kids)
    if _is(n, 'or'):
        items = _flatten_or(n)
        return len(items) + sum(k_input_cost_tree(k) for k in items)
    raise ValueError

# ───────────────────── DAG (with sharing) helpers ───────────────────────
def _canon(n):
...
def k_input_cost_dag(n):
    """AND/OR with k inputs costs k; NOT costs 0. With common subexpression sharing."""
    seen = set()
    def dfs(x):
        key = _canon(x)
        ...
        if _is(x, 'not'): return dfs(x[1])  # NOT costs 0
        if _is(x, 'and'):
            kids = _flatten_and(x)
            return len(kids) + sum(dfs(k) for k in kids)
        if _is(x, 'or'):
            items = _flatten_or(x)
            return len(items) + sum(dfs(k) for k in items)
        raise ValueError
    return dfs(n)
...
def analyze_multilevel(tree):
    """
    Returns {'tree': {...}, 'dag': {...}} with metrics:
      - cost 
      - gate_counts
      - depth_no_not / depth_with_not
      - literal_occurrences
    """
    t = nnf(tree)  # push NOTs to leaves for fair counting
    stats_tree = {
        'cost':     k_input_cost_tree(t),      
...
def _multilevel_cost(stats, prefer='dag'):
    """
    Lexicographic score tuple used for ranking:
      (cost, depth_no_not, literal_occurrences)
    Choose DAG vs TREE via 'prefer'.
    """
    s = stats['dag'] if prefer == 'dag' else stats['tree']
    return (s['cost'],
            stats['tree']['depth_no_not'],
            stats['tree']['literal_occurrences'])

def get_expression_stats(expr: str):
    """Parse a boolean expression string and compute metrics."""
